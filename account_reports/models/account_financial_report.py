...
class ReportAccountFinancialReport(models.Model):
    ...
    def filter_analytic(self):
        enable_filter_analytic_accounts = self.env.user.id in self.env.ref(
            'analytic.group_analytic_accounting').users.ids
        enable_filter_analytic_tags = self.env.user.id in self.env.ref(
            'analytic.group_analytic_tags').users.ids
    ...
    line_ids = fields.One2many(
        'account.financial.html.report.line', 'financial_report_id', string='Lines')
    ...
    comparison = fields.Boolean(
        'Allow comparison', default=True, help='display the comparison filter')
    analytic = fields.Boolean('Allow analytic filters',
                              help='display the analytic filters')
    show_journal_filter = fields.Boolean(
        'Allow filtering by journals', help='display the journal filter in the report')
    unfold_all_filter = fields.Boolean(
        'Show unfold all filter', help='display the unfold all options in report')
    ...
    parent_id = fields.Many2one(
        'ir.ui.menu', related="generated_menu_id.parent_id", readonly=False)
    tax_report = fields.Boolean(
        'Tax Report', help="Set to True to automatically filter out journal items that have the boolean field 'tax_exigible' set to False")
    ...
    def _init_filter_ir_filters(self, options, previous_options=None):
        ...
        if previous_options and previous_options.get('ir_filters'):
            filters_map = dict((opt['id'], opt['selected'])
                               for opt in previous_options['ir_filters'])
        else:
            filters_map = {}
        options['ir_filters'] = []
    ...
    def _get_options(self, previous_options=None):
        # OVERRIDE
        options = super(ReportAccountFinancialReport,
    ...
    def _build_lines_hierarchy(self, options_list, financial_lines, solver, groupby_keys):
        ...
        for financial_line in financial_lines:
            ...
            if self.env.company.totals_below_sections and (financial_line.children_ids or (is_leaf and financial_report_line['unfolded'] and aml_lines)):
                lines.append(self._get_financial_total_section_report_line(
                    options_list[0], financial_report_line))
                # enables adding "o_js_account_report_parent_row_unfolded" -> hides total amount in head line as it is displayed later in total line
    ...
    def _build_headers_hierarchy(self, options_list, groupby_keys):
        ...
        sorting_map = [{i: (i, self.format_date(options))
        ...
        for groupby, ids_set in zip(groupby_list, keys_grouped_by_ids):
            ...
            if groupby_field.relational:
                ...
                if groupby_field.name == 'date':
                    def format_func(v): return fields.Date.to_string(v)
                elif groupby_field.name == 'datetime':
                    def format_func(v): return fields.Datetime.to_string(v)
                else:
                    def format_func(v): return v
        ...
        def _create_headers_hierarchy(level_keys, level=0):
            ...
            headers = sorted(
                headers, key=lambda header: sorting_map[level][header['key']][0])
            return headers
        ...
        def _populate_headers(current_node, current_key=[], level=0):
            ...
            for header in current_node:
                ...
                if children:
                    _populate_headers(children, current_key +
                                      [header['key']], level=level + 1)
                else:
        ...
        for i in range(len(headers)):
            headers[i] = [
                {'name': '', 'class': 'number', 'colspan': 1}] + headers[i]
    ...
    def _get_lines(self, options, line_id=None):
        ...
        financial_line = self.env['account.financial.html.report.line'].browse(
            line_id)
        formula_solver = FormulaSolver(options_list, self)
        formula_solver.fetch_lines(financial_line)
        sorted_groupby_keys = [tuple(key)
                               for key in options['sorted_groupby_keys']]
        lines = self._build_lines_hierarchy(
            options_list, financial_line, formula_solver, sorted_groupby_keys)
    ...
    def _get_table(self, options):
        ...
        options_list = self._get_options_periods_list(options)
        formula_solver = FormulaSolver(options_list, self)
        financial_lines = self.env['account.financial.html.report.line'].search(
            [('id', 'child_of', self.line_ids.ids)])
        formula_solver.fetch_lines(financial_lines)
        groupby_keys = formula_solver.get_keys()
        headers, sorted_groupby_keys = self._build_headers_hierarchy(
            options_list, groupby_keys)
        lines = self._build_lines_hierarchy(
            options_list, self.line_ids, formula_solver, sorted_groupby_keys)
    ...
    def _get_financial_line_report_line(self, options, financial_line, solver, groupby_keys):
        ...
        is_leaf = solver.is_leaf(financial_line)
        has_lines = solver.has_move_lines(financial_line)
        has_something_to_unfold = is_leaf and has_lines and bool(
            financial_line.groupby)
        ...
        for key in groupby_keys:
            ...
            columns.append({'name': self._format_cell_value(
                financial_line, amount), 'no_format': amount, 'class': 'number'})
        ...
        if self._display_debug_info(options):
            columns.append(self._compute_debug_info_column(
    ...
    def copy(self, default=None):
        ...
        copied_report_id = super(
            ReportAccountFinancialReport, self).copy(default=default)
        for line in self.line_ids:
            line._copy_hierarchy(
                report_id=self, copied_report_id=copied_report_id)
    ...
    def _create_action_and_menu(self, parent_id):
        ...
        for report in self:
            if not report.generated_menu_id:
                ...
                action_xmlid = "%s.%s" % (
                    module, 'account_financial_html_report_action_' + str(report.id))
                data = dict(xml_id=action_xmlid,
                            values=action_vals, noupdate=True)
                action = self.env['ir.actions.client'].sudo()._load_records([
                    data])
                ...
                menu_xmlid = "%s.%s" % (
                    module, 'account_financial_html_report_menu_' + str(report.id))
...
class AccountFinancialReportLine(models.Model):
    ...
    financial_report_id = fields.Many2one(
        'account.financial.html.report', 'Financial Report')
    parent_id = fields.Many2one(
        'account.financial.html.report.line', string='Parent', ondelete='cascade')
    children_ids = fields.One2many(
        'account.financial.html.report.line', 'parent_id', string='Children')
    ...
    print_on_new_page = fields.Boolean(
        'Print On New Page', help='When checked this line and everything after it will be printed on a new page.')
    green_on_positive = fields.Boolean(
        'Is growth good when positive', default=True)
    ...
    special_date_changer = fields.Selection([
        ...
        # So, the start of the accounting if include_initial_balance is True on the account type
    ...
    show_domain = fields.Selection(
        [('always', 'Always'), ('never', 'Never'), ('foldable', 'Foldable')], default='foldable')
    ...
    def _check_line_consistency(self):
        ...
        for rec in self:
            ...
            if rec.code:
                protected_codes = set(dir(__builtins__)).union(
                    set(PROTECTED_KEYWORDS))
                if rec.code and rec.code.strip().lower() in protected_codes:
                    raise ValidationError("The code '%s' is invalid on line with name '%s'") % (
                        rec.code, rec.name)
            ...
            if rec.groupby:
                ...
                if not groupby_field or not self.env['account.financial.html.report']._is_allowed_groupby_field(groupby_field):
                    raise ValidationError(
                        _("Groupby field %s is invalid on line with name '%s'") % (self.groupby, rec.name))
    ...
    def _get_options_financial_line(self, options, calling_financial_report=None, parent_financial_report=None):
        ...
        if not is_same_report and not calling_financial_report._context.get('preserve_same_options'):
            new_options = parent_financial_report._get_options(
        ...
        if self.special_date_changer == 'strict_range':
            ...
            new_options['date'].update(
                {'date_from': False, 'date_to': date_to, 'strict_range': False})
            ...
            new_options['date'].update(
                {'date_from': date_from, 'date_to': date_to, 'strict_range': True, 'mode': 'range'})
        return new_options
    ...
    def _compute_amls_results(self, options_list, calling_financial_report=None, sign=1):
        ...
        horizontal_groupby_list = AccountFinancialReportHtml._get_options_groupby_fields(
            options_list[0])
        groupby_list = [self.groupby] + horizontal_groupby_list
        groupby_clause = ','.join('account_move_line.%s' %
                                  gb for gb in groupby_list)
        ...
        for i, options in enumerate(options_list):
            new_options = self._get_options_financial_line(
                options, calling_financial_report, parent_financial_report)
            line_domain = self._get_domain(
                new_options, parent_financial_report)

            tables, where_clause, where_params = AccountFinancialReportHtml._query_get(
        ...
        parent_financial_report._cr_execute(
            options_list[0], ' UNION ALL '.join(queries), params)
    ...
    def _compute_sum(self, options_list, calling_financial_report=None):
        ...
        * (1,3): At the period 1 (first comparison), the results for 'partner_id = 3' are...
        ...
        groupby_in_formula = any(x in (self.formulas or '')
                                 for x in ('sum_if_pos_groupby', 'sum_if_neg_groupby'))
        if groupby_in_formula and self.groupby and self.groupby not in all_groupby_list:
            all_groupby_list.append(self.groupby)
        groupby_clause = ','.join('account_move_line.%s' %
                                  gb for gb in all_groupby_list)
    ...
    def _copy_hierarchy(self, report_id=None, copied_report_id=None, parent_id=None, code_mapping=None):
        ...
        for line in self.children_ids:
            line._copy_hierarchy(parent_id=copy_line_id,
        ...
        if self.formulas:
            ...
            for k, v in code_mapping.items():
                for field in ('debit', 'credit', 'balance'):
                    suffix = '.' + field
                    copied_formulas = copied_formulas.replace(
                        k + suffix, v + suffix)
    ...
    def action_view_journal_entries(self, options, calling_financial_report_id=None):
        ...
        self.ensure_one()
        parent_financial_report = self._get_financial_report()
        calling_financial_report = self.env['account.financial.html.report'].browse(
            calling_financial_report_id)
        new_options = self._get_options_financial_line(
            options,
            calling_financial_report=calling_financial_report,
            parent_financial_report=parent_financial_report,
        )
        domain = self._get_domain(new_options, parent_financial_report) + \
            parent_financial_report._get_options_domain(new_options)
        return {
