...
class AccountCashFlowReport(models.AbstractModel):
    ...
    def _get_liquidity_move_report_lines(self, options, currency_table_query, payment_move_ids, payment_account_ids):
        ...
        query = '''
                ...
                AND credit_line.branch_id in %s AND credit_line.operating_unit_id in %s
                ...
                AND partial.max_date BETWEEN %s AND %s
                AND debit_line.branch_id in %s AND debit_line.operating_unit_id in %s
                GROUP BY debit_line.company_id, debit_line.account_id, account.code, account_name, account.internal_type
            '''
        self._cr.execute(query, [
            self.env.user.lang, payment_move_ids, payment_account_ids, options['date']['date_from'],
            options['date']['date_to'],
            tuple(branch), tuple(operating_unit),
            self.env.user.lang, payment_move_ids, payment_account_ids, options['date']['date_from'],
            options['date']['date_to'],
            tuple(branch), tuple(operating_unit)
        ])
        ...
        query = '''
                ...
                AND line.branch_id in %s AND line.operating_unit_id in %s
                GROUP BY line.account_id, account.code, account_name, account.internal_type
            '''
        self._cr.execute(query, [self.env.user.lang, payment_move_ids, payment_account_ids, tuple(branch), tuple(operating_unit)])
    ...
    def _get_reconciled_move_report_lines(self, options, currency_table_query, payment_move_ids, payment_account_ids):
        ...
        query = '''
            ...
            '''
        self._cr.execute(query, [self.env.user.lang, tuple(reconciled_percentage_per_move.keys()), tuple(branch),
                                 tuple(operating_unit)])
