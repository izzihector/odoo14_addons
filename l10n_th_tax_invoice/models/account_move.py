...
class AccountMoveTaxInvoice(models.Model):
    ...
    period_date = fields.Date(string="Period Date", copy=False)
    ...
    @api.depends("report_late_mo", "period_date")
    def _compute_report_date(self):
        for rec in self:
            if rec.period_date:
                eval_date = rec.period_date + relativedelta(
                ...
                taxinv = TaxInvoice.create(
                    {
                        ...
                        "period_date": sign < 0 and fields.Date.today() or False,
...
class AccountMove(models.Model):
    ...
    same_ref_move_id = fields.Many2one('account.move', string='Move with same tax number',
                                       compute='_compute_same_ref_move_id', store=False)
    undue_input_vat = fields.Boolean('Undue Input Vat')

    @api.depends('ref')
    def _compute_same_ref_move_id(self):
        self.same_ref_move_id = False
        for move in self:
            move_id = move._origin.id
            partner_id = move.partner_id
            Move = self.with_context(active_test=False).sudo()
            if move.move_type in ('in_invoice', 'in_receipt', 'in_refund') and move.ref:
                domain = [
                ...
                move.same_ref_move_id = Move.search(domain, limit=1) or False

    @api.onchange('ref')
    def onchange_ref(self):
        for move in self:
            for tax in move.tax_invoice_ids:
                tax.tax_invoice_number = move.ref
    ...
    def _post(self, soft=True):
        ...
        for move in self:
            for tax_invoice in move.tax_invoice_ids.filtered(
                ...
                if ((
                        not tax_invoice.tax_invoice_number or not tax_invoice.tax_invoice_date or not tax_invoice.period_date) and not move.undue_input_vat):
                    if tax_invoice.payment_id:  # Defer posting for payment
                        ...
                        raise UserError(_("Please fill in tax invoice, tax date and period date"))
        ...
        for move in self:
            for tax_invoice in move.tax_invoice_ids.filtered(
                ...
                tax_invoice.write(
                    {"tax_invoice_number": tinv_number, "tax_invoice_date": tinv_date, 'period_date': tinv_date}
                )
