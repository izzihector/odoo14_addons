...
odoo.define('bi_pos_stock.pos', function(require) {
	...
	screens.ProductScreenWidget.include({
		start: function() {
			var self = this;
			var product_dict = self.pos.db.product_by_id;
			...
			{	
				...
				if (self.pos.config.pos_stock_type == 'onhand')
				{
					rpc.query({
							...
							for(let prod in product_dict){
								product_dict[prod]['bi_on_hand'] = product_dict[prod].qty_available;
								product_dict[prod]['bi_available'] = product_dict[prod].virtual_available;
								for(let key in self.pos.loc_onhand)
								{
									if(prod == key)
									{
										product_dict[prod]['bi_on_hand'] = self.pos.loc_onhand[key];
					...
					rpc.query({
						...
						for(let prod in product_dict){
							...
							for(let key in self.pos.loc_available)
							{
								if(prod == key)
								{
									product_dict[prod]['bi_available'] = self.pos.loc_available[key];
