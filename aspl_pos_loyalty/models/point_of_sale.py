...
class pos_order(models.Model):
    ...
    @api.model
    def loyalty_config_read(self):
        config = self.env['loyalty.config.settings'].get_values()
        loyalty_config = {}
        if config:
            loyalty_config['points_based_on'] = config['points_based_on']
            loyalty_config['minimum_purchase'] = config['minimum_purchase']
            loyalty_config['point_calculation'] = config['point_calculation']
            loyalty_config['points'] = config['points']
            loyalty_config['to_amount'] = config['to_amount']
    ...
    def _process_order(self, order, draft, existing_order):
        ...
        orders = order['data']
        pos_session = self.env['pos.session'].browse(orders['pos_session_id'])
        partner = self.env['res.partner'].browse(orders['partner_id'])
        if pos_session.config_id.enable_pos_loyalty and partner:
            loyalty_setting_id = self.env['loyalty.config.settings'].get_values()
            if loyalty_setting_id['points_based_on'] and orders['loyalty_earned_point']:
                point_vals = {
                    'pos_order_id': res,
                    'partner_id': partner.id,
                    'points': orders['loyalty_earned_point'],
                    'amount_total': (float(orders[
                        'loyalty_earned_point']) * loyalty_setting_id['to_amount']) / loyalty_setting_id[
                                        'points']
                }
                loyalty = self.env['loyalty.point'].create(point_vals)
                if loyalty:
                    if partner.send_loyalty_mail:
                        try:
                            template_id = self.env['ir.model.data'].get_object_reference('aspl_pos_loyalty',
            ...
            if orders['loyalty_redeemed_point']:
                redeemed_vals = {
                    'redeemed_pos_order_id': res,
                    'partner_id': partner.id,
                    'redeemed_amount_total': self._calculate_amount_total_by_points(loyalty_setting_id, orders[
                        'loyalty_redeemed_point']),
                    'redeemed_point': orders['loyalty_redeemed_point'],
                }
