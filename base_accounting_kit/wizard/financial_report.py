    ...
    def view_report_pdf(self):
        ...
        data['form'] = self.read(
            ['date_from', 'enable_filter', 'debit_credit', 'date_to',
             ...
             'company_id', 'branch_id', 'operating_unit_id'])[0]
        print(data['form'])
        ...
        print('journal_items11', journal_items)
        ...
        for i in report_lines:
            ...
            if i['type'] == 'account':
                ...
                if flag:
                    ...
                    vals = [account]
                if form['branch_id']['branch_id']:
                    search_query += " and aml.branch_id=%s"
                    print('branhc', search_query)
                    vals += [form['branch_id']['branch_id'][0]]

                if form['operating_unit_id']['operating_unit_id']:
                    search_query += " and aml.operating_unit_id=%s"
                    print('unit1111111', search_query)
                    vals += [form['operating_unit_id']['operating_unit_id'][0]]
                    print(vals, 'calssssss')

                if form['date_from'] and form['date_to']:
                    search_query += " and aml.date>=%s and aml.date<=%s"
                    vals += [form['date_from'], form['date_to']]
                elif form['date_from']:
